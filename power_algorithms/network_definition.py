...
def create_network():
    ...
    for i in range(0, 100):
        ...
        pp.create_load(network, bus=lowVoltageBusNodes[i], p_mw=0.07, q_mvar=0.025, name="Load_" + str(i+1))
    ...
    for i in range(0, 99):
        if (i == 80):
            pp.create_line(network, from_bus=30, to_bus=busNodes[i+1], length_km=0.2, name="Line_" + str(i+1), std_type="NA2XS2Y 1x150 RM/25 12/20 kV")
        else:
    ...
    pp.create_shunt_as_capacitor(network, pp.get_element_index(network, "bus", 'Bus_Cap2'), 0.125, 0, name="Cap2")
    ...
    pp.create_shunt_as_capacitor(network, pp.get_element_index(network, "bus", 'Bus_Cap7'), 0.6, 0, name="Cap7")
    ...
    pp.create_shunt_as_capacitor(network, pp.get_element_index(network, "bus", 'Bus_Cap8'), 0.6, 0, name="Cap8")
